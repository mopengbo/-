...
def show_pic(img, bboxes=None,j=0):
    '''
    输入:
        img:图像array
    ...
    cv2.imwrite('./data_mask/MaskDataset/VOC/val/JPEGImages/train_auged_'+str(j)+'.jpg', img)

    #img = cv2.imread('../data_mask/MaskDataset/VOC/val/JPEGImages/train_auged_'+str(j)+'.jpg')
   ...
   # os.remove('../data_mask/MaskDataset/VOC/val/JPEGImages/train_auged_'+str(j)+'.jpg')
...
def generate_xml(img_name,coords,img_size,out_root_path):
    ...
    # 将DOM对象doc写入文件
    #f = open(os.path.join(out_root_path, img_name+'.xml'),'w')
...
if __name__ == '__main__':
    ...
    source_pic_root_path = '../data_mask/MaskDataset/VOC/val/JPEGImages'
    source_xml_root_path = '../data_mask/MaskDataset/VOC/val/Annotations'
    ...
    for parent, _, files in os.walk(source_pic_root_path):
        for file in files:
            ...
            while cnt < need_aug_num:
                ...
                print(auged_bboxes)
                cnt += 1
                size = auged_img.shape

                show_pic(auged_img, auged_bboxes,i)  # 强化后的图
                generate_xml("train_auged_"+str(i), auged_bboxes, size, '../data_mask/MaskDataset/VOC/val/Annotations/')
                i = i+1